...
import styles from './Rusu.module.css';
...
export default function RusuPage() {
    ...
    useEffect(() => {
        if (likes.length > 0) {
            localStorage.setItem('rusuLikes', JSON.stringify(likes));
        }
        if (dislikes.length > 0) {
            localStorage.setItem('rusuDislikes', JSON.stringify(dislikes));
        }
    }, [likes, dislikes]);
